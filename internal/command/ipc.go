...
func printUsage() {
	...
	fmt.Fprintf(os.Stderr, "  fluxid {--claude|--codex|--opencode} --file=PATH [options] [agent-args]\n")
	...
	fmt.Fprintf(os.Stderr, "  (default)                Run workflow controller (requires agent flag)\n")
	...
	fmt.Fprintf(os.Stderr, "\nAgent Options:\n")
	fmt.Fprintf(os.Stderr, "  --claude                      Use Claude agent\n")
	fmt.Fprintf(os.Stderr, "  --codex                       Use Codex agent\n")
	fmt.Fprintf(os.Stderr, "  --opencode                    Use OpenCode agent\n")
	fmt.Fprintf(os.Stderr, "\nWorkflow Options:\n")
	fmt.Fprintf(os.Stderr, "  --file PATH                   Absolute path to task file (required)\n")
	fmt.Fprintf(os.Stderr, "  --config PATH                 Custom config file path\n")
	...
	fmt.Fprintf(os.Stderr, "  --dry-run                     Alias for --fluxid-dry-run\n")
	fmt.Fprintf(os.Stderr, "  --fluxid-output FORMAT        Set initialization output format (text|json|yaml)\n")
	fmt.Fprintf(os.Stderr, "  --output FORMAT               Alias for --fluxid-output\n")
	fmt.Fprintf(os.Stderr, "\nCommand Overrides:\n")
	fmt.Fprintf(os.Stderr, "  --implement-command PATH      Override implement command from config\n")
	fmt.Fprintf(os.Stderr, "  --review-command PATH         Override review command from config\n")
	fmt.Fprintf(os.Stderr, "  --commit-command PATH         Override commit command from config\n")
	fmt.Fprintf(os.Stderr, "\nOther Options:\n")
