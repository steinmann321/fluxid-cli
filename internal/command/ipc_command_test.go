	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if testCase.envValue != "" {
				t.Setenv("FLUXID_SESSION_ID", testCase.envValue)
			} else {
				_ = os.Unsetenv("FLUXID_SESSION_ID")
			}

			// Parse session flag
			sessionID, err := parseSessionFlag(testCase.args)

			// Check error
			if (err != nil) != testCase.wantErr {
				t.Errorf("parseSessionFlag() error = %v, wantErr %v", err, testCase.wantErr)
				return
			}

			// Check session ID
			if sessionID != testCase.wantSession {
				t.Errorf("parseSessionFlag() sessionID = %q, want %q", sessionID, testCase.wantSession)
			}
		})
	}
}
