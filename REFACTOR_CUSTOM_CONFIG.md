# Custom Config Refactoring - Complete Plan

**Status:** APPROVED - Ready for Implementation
**Date:** 2025-12-25
**Version:** 2.0 (Breaking Changes)

This document contains the complete refactoring plan for implementing Spring Boot-style configuration in fluxid with custom config file support, equals-only CLI syntax, and workflow simplifications.
...
### Goals
1. **Spring Boot-style configuration**: Support `--config` flag to load custom config files
2. **Clear precedence chain**: Default config → Custom config (--config) → CLI flags
3. **Simplify CLI**: Use equals syntax only (`--flag=value`), remove environment variables
4. **Simplify workflow**: Make commits mandatory (always run)
5. **Clean codebase**: Remove source tracking, remove commit toggle logic

### Breaking Changes (v1.x → v2.0)
- ✗ Environment variables removed (FLUXID_ITERATIONS, etc.)
- ✗ Config files now REQUIRED (at least one must exist)
